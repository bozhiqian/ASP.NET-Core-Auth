    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> AdditionalAuthenticationFactor(AdditionalAuthenticationFactorViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var user = UserRepository.GetUserBySubjectId(tempUser.GetSubjectId());
        ...
        public async Task<IActionResult> ExternalLogin(string provider, string returnUrl)
        {
            ...
            else
            {
                ...
                return Challenge(props, provider); // Redirect to Facebook when you aren't logged into Facebook. After consent from Facebook, it will redirect to ExternalLoginCallback(). 
            }
        }
        ...
        public async Task<IActionResult> ExternalLoginCallback()
        {
            ...
            #region map the external user to Entities.User (Customized code)

            // lookup our user and external provider info
            var (provider, providerUserId, claims) = result.FindUserFromExternalProvider();
            ...
            var user = UserRepository.GetUserByProvider(provider, providerUserId);
            ...
            if (user == null)
            {
                ...
                if (email != null)
                {
                    var userByEmail = UserRepository.GetUserByEmail(email.Value);
                    if (userByEmail != null)
                    {
                        // add external login for this user, so this user might have multiple user logins. 
                        UserRepository.AddUserLogin(userByEmail.SubjectId, provider, providerUserId);
                ...
                var continueWithUrl = Url.Action("RegisterUser", "UserRegistration", new { returnUrl = returnUrlAfterRegistration, provider = provider, providerUserId = providerUserId });
                return Redirect(continueWithUrl);
            }

            #endregion

            #region Commented original code
